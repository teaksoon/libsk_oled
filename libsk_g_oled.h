// Library: libsk_g_oled ( "G" Version - Graphics and Text )
// Arduino Library for SSD1306 OLED i2c 68x128pixel Display Module
// by TeakSoon Ding for STEMKRAF ( OCT-2021 ) - Release 1
//
// This STEMKRAF OLED Library is designed to be as small and as simple as possible. 
// In order to do that, this library is kept to have very few functions, no error 
// corrections or support for non AVR micro-controllers or other display modules. 
// Although it may sound limiting, it is enough for most task. This Library has 2 
// Versions, the "C" Version and the "G" Version.
// - "C" ( "libsk_c_oled.h" ) - "Text" only ( Does not use SRAM Memory )
// - "G" ( "libsk_g_oled.h" ) - Graphics and "Text" ( Uses 1024 bytes of SRAM Memory )
// Note: atmega328 micro-control only have 2048 bytes of SRAM Memory
//
// For "Text" functions, the STEKRAF OLED library has the following basic functions,
//
...
// NOTE: Since very few error correction are available, use within the following limits for
// "Text" functions.
// _row - (0 to 8 - top to bottom ), for Char and Bitmap
//      - (0 to 7 - top to bottom ), for "L" Char ( uses 2 rows each )
// _col - (0 to 127 pixel - left to right), for Bitmap ( each Byte of Bitmap is 1 pixel )
//      - (0 to 123 pixel - left to right), for Char ( each Char is 5-pixel ) 
//      - (0 to 118 pixel - left to right), for "L" Char ( each "L" Char is 10-pixel ) 
// _cha     - ASCII Char ( 32 to 126 )
// _str     - Array of Chars, terminated by NULL 
//          - 1-pixel gap between the Chars, 2-pixel gap between "Large" Chars
...
//
// This is the "G" Version of the STEMKRAF OLED Library, it has the following
// additional Graphics function,
//
// 8.sk_oled_showDot(_row,_col,_state ) - display a single dot on the screen
// 9.sk_oled_showHLine(_row,_col,_len,_state) - Draw Horizontal Line
// 10.sk_oled_showVLine(_row,_col,_len,_state) - Draw Vertical Line
// 11.sk_oled_showCircle(_row,_col,__radius,_state) - Draw a Circle
// 12.sk_oled_showLine(_row_1,_row_2,_col_1,_col_2,_state) - Draw a Diagonal Line
// 13.sk_oled_showRadialSpokes(_row,_col,_len,_len_idx,_step,__step_idx,_state) - Draw Radial Spokes
// NOTE: Since very few error correction are available, use within the following limits for the
// Graphics function ( please take note the row, it is 0 to 63 for Graphics Mode )
...
// _step_idx - Index of step ( 0 to _step-1 ), step_idx=0 is vertical top from center, 
//           - step_idx=1... is next, in clockwise movement, and so on
//
...
// - for "G" version of the STEMKRAF OLED Lirary
...
// --- Instalation DONE!!! ---  
//
// Example of a basic Arduino Program with STEMKRAF OLED library: 
// ---
